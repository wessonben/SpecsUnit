...
namespace SpecsUnit.AutoMocker
{
    /// <summary>
    ...
    public class AutoMocker
    {
        ...
        public Mock<TService> GetMock<TService>() where TService : class
        {
            ...
            if (!instance.IsMock)
                throw new ArgumentException(string.Format("Registered service `{0}` was not a mock",
                    Get<TService>().GetType()));
            ...
            where TService : class
        {
            ...
            if (CastChecker.DoesContainCastToObject(expression))
            {
                throw new NotSupportedException(
                    "Use the Setup overload that allows specifying TReturn if the setup returns a value type");
            }
